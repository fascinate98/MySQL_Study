...
--                  SMITH        MIT         SM               ITH
-- 	 	:	:        :                   :
select ename, mid(ename, 2, 5), left(ename, 2), right(ename, 3) from emp;
...
select ename "사원명" , concat(left(lower(ename), 1), right(lower(ename), 1)) "결과" from emp;
...
-- 	148,700원
-- 	   :
select concat(format(sal * 50, -2), '원') '계산결과' from emp;
...
-- 	SMITH	미정	
-- 	ALLEN	300	
select ename, ifnull(comm, '미정') '결과' from emp;
...
-- 	ALLEN	C부서	
-- 	:   
select ename, empno, case deptno
when '10' then "A 부서"
when '20' then "B 부서"
when '30' then "C 부서"
when '40' then "D 부서"
...
-- 10. 오늘날짜와 오늘날짜에서 10일을 더한 날짜를 출력하시오.  //curdate = s날짜만 //now : 시간도
select curdate(), date_add(curdate(), interval 10 day ) from dual;
...
-- 	WARD             1250                 17400
--    			:
select ename '이름' , sal '월급여' , 12 * (200 + sal) '연봉' from emp;
...
when sal <= 1000 and sal > 0 then 'A'
when sal <= 2000 and sal > 1001 then 'B'
when sal <= 3000 and sal > 2001 then 'C'
when sal <= 4000 and sal > 3001 then 'D'
...
from emp order by  timestampdiff(month, hiredate, now()) desc;
...
-- ALLEN    14909일
--      :
select ename , concat(datediff(now(), hiredate), '일') '근무일수' from emp ;
...
select weekday('1998-12-15');


-- 19. 내생일을 기준으로 요일을 출력하는 SQL  명령을 작성하시오.(요일을 요일명으로)
SELECT case WEEKDAY('1998-12-15') 
when '0' then '월요일' 
when '1' then '화요일' 
when '2' then '수요일' 
when '3' then '목요일' 
...
select timestampdiff(month, '1998-12-15' , now());


-- 21. 사원테이블에서 이름의 첫글자가 A이고 마지막 글자가 N이 아닌 사원의
-- 이름을 출력하시오.
select ename from emp where left(ename ,  1) = 'A' and  right(ename, 1) != 'N';
...
--  EMPNO ENAME    SAL
---------- ------ ----------
-- 7839	KING	5,000원    서브쿼리..
-- 7788	SCOTT	3,000원
-- 7902	FORD	3,000원
-- 7566	JONES	2,975원
-- 7698	BLAKE	2,850원
-- 7782	CLARK	2,450원
select empno, ename, concat(format(sal, 0), '원') from emp order by sal desc;
...
select ename, sal, deptno, year(hiredate) from emp where 
year(hiredate) in (select year(hiredate) from emp where DEPTNO = '30') and deptno != '30' 
 order by sal;
...
ename != (select ename from emp where ename = 'BLAKE') ;
...
deptno =ANY (select deptno from emp where ename like '%S%') ;
...
-- FORD	20	3000
-- KING	10	5000
select ename, deptno, sal from emp where sal >all (select max(sal) from emp where deptno = '30') ;
...
-- 30번 부서  JAMES	       CLERK
 select concat(deptno,  '번 부서'), ename, job from emp where
...
-- BLAKE	   1981년 05월 01일
-- CLARK   1981년 06월 09일
select ename, hiredate, mgr from emp where 
